    ...
    <section className="py-16 px-4 md:px-8 lg:px-16 bg-background">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PROMOS.map((p) => (
            <div key={p.title} className="bg-card border border-border rounded-xl p-6 hover:border-primary transition-all duration-300 flex flex-col">
              <h3 className="font-heading text-lg font-bold text-primary mb-2">{p.title}</h3>
              <p className="text-sm text-muted-foreground mb-4 flex-1">{p.desc}</p>
              <Link to={p.link} className="text-xs text-muted-foreground hover:text-primary mb-3 underline">Learn more</Link>
              <Link
                to={p.cta}
                className="bg-primary text-primary-foreground font-bold rounded-lg px-4 py-2.5 text-sm text-center hover:bg-gold-hover transition-all"
              >
                {p.ctaText}
              </Link>
            </div>

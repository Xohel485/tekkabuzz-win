...
import { IMAGES, PAYMENT_ICONS, TRUST_ICONS } from "@/lib/images";
    ...
    <footer className="bg-secondary border-t border-border">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          {/* Brand */}
          <div>
            <img src={IMAGES.LOGO_FOOTER} alt="TekkaBuzz Footer Logo" className="h-12 mb-4 object-contain" />
          ...
          {/* Games */}
          <div>
            <h3 className="font-heading text-sm font-bold text-primary mb-4 tracking-wider uppercase">Games</h3>
          ...
          {/* Legal */}
          <div>
            <h3 className="font-heading text-sm font-bold text-primary mb-4 tracking-wider uppercase">Legal</h3>
            <ul className="space-y-2">
              {LEGAL_LINKS.map((l) => (
                <li key={l.to}>
                  <Link to={l.to} className="text-sm text-muted-foreground hover:text-primary transition-colors">{l.label}</Link>
        ...
        <div className="border-t border-border pt-8 mb-8">
          <div className="flex flex-wrap gap-4 justify-center items-center">
            {PAYMENT_ICONS.map((p) => (
              <img key={p.name} src={p.url} alt={p.name} className="h-8 object-contain opacity-70 hover:opacity-100 transition-opacity" />
            ))}
          </div>
        </div>
